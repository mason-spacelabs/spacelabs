...
var collectData = function(customer){
    var parent = document.getElementsByClassName('cart-item');
    var productArray = [];
    var shopifyCustomer = {
        customer: customer
    }
    ...
    for (var i = 0; i < parent.length; i++) {
        var mfgProduct = parent[i].attributes["4"].value;
        var shopifyProduct = parent[i].attributes["2"].value;

        shopifyCustomer.variants = JSON.stringify({mfgProduct : shopifyProduct}); 
        productArray.push(mfgProduct);
    }
    var productList = productArray.join(",");
    var customerNumber = "90022447";

    var requestParameters = customerNumber + "&" + productList; 
    var requestObject = {
        data: requestParameters
    };

    spacelabsPricing(requestObject);
};
var spacelabsPricing = function(parameters) {
    $.ajax({
        url: "https://4bf3eea2.ngrok.io/spacelabs/pricing", 
        type: "POST",
        crossDomain: true,
        data: parameters,
        dataType: "json",
...
var submitRequest = function(form) {
    ...
    var promise = new Promise(function(resolve, reject){
        ...
        $.ajax({
            url: "https://4bf3eea2.ngrok.io/spacelabs/register", 
            type: "POST",
            crossDomain: true,
            data: form,
            dataType: "json",
