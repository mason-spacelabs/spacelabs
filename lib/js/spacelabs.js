...
var NGROK = "https://5914f333.ngrok.io";  
...
var collectData = function(customer, update){

    $(".loading-screen").css( "display", "block" ); //Loading Screen

    var parent = document.getElementsByClassName('cart-item'); // Grabbing car content
    var productArray = []; // Request string contents
    var customerNumber = "90022447"; // This is the GPO Account number, Once added will need to be replaced
    var ShopifyCustomer = {
        ShopifyCustomerNumber: customer,
        Variants: {
        }
    } 
    ...
    for (var i = 0; i < parent.length; i++) {
        ...
        var productIdentifiers = parents.getAttribute("data-cartitem-id"); // Product Identifiers for API Admin Post
        var productNames = parents.getAttribute("data-cartitem-sku"); // Product names for MFG PRO
        
        ShopifyCustomer.Variants[productNames] = productIdentifiers;
        productArray.push(productNames); 
    }

    var productList = productArray.join(",");
    var requestParameters = customerNumber + "&" + productList; // Creating the request string for MFG PRO
    ShopifyCustomer.RequestString = requestParameters; // Adding request string to the shopifyCustomer Object

    spacelabsPricing(ShopifyCustomer, update);
};
var spacelabsPricing = function(parameters, update) {
    $.ajax({
        url: "/apps/ecommerce", 
        type: "POST",
        data: parameters,
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            ...
            var pricing_loading = document.querySelectorAll("#loading-modal");
            var checkout_error = document.querySelectorAll("#checkout_options");

            pricing_loading[0].classList.add('hidden');
            checkout_error[0].classList.remove('hidden');
...
var submitRequest = function(form) {
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                ...
                if(errorThrown != "Unprocessable Entity"){
                    ...
                    text_header_element.appendChild(text_header);
 ...
 function errorCart(){
    ...
    checkout_error[0].classList.add('hidden');
    $(".loading-screen").css( "display", "none" ); //Loading Screen
 }